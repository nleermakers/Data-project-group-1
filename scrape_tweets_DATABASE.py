...
conn_string = "host='localhost' dbname='postgres' user='postgres' password='...'port='5432'"
data_location = 'ids.csv.txt'
tweets_table = "pauw_new_data"
users_table = "pauw_new_data"
################################################################################


# EDIT THE STRINGS TO USE YOUR API KEYS OBTAINED FROM ('https://apps.twitter.com/')
CONSUMER_KEY = 'foBEOuS6GLJsAwHN1gLsNoiBG'
CONSUMER_SECRET = 'UBvTBH6UpG3ivwMD6n3HCfhqxksk5xcCjJMhm4DZgtERKONd4I'
ACCESS_TOKEN = '928555297098158080-hzYKsewGpHZ9MORanZeiwUPWhqBzj7P'
ACCESS_TOKEN_SECRET = 'yM00i0oxp8XIpHCbJMJbrGnXa5M0Qs0dMDRpVcEQjUllv'
...
for i in range(c, iterations):
    ...
    try:
        ...
        # every time the loop starts again, the latitude and longitude are set to 0, so coordinates are not copied.
            longitude = 0
            latitude = 0
    ...
    # defining the attributes we want to scrape
            try:
                cur = conn.cursor()
                cur.execute("INSERT INTO " + users_table + "(tweetid, created_at, hashtags, text, longitude, latitude, name, screen_name, location, description) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                            (tweetid, tweet_created_at, json.dumps(hashtags), text, longitude, latitude, name, screen_name, location, description))
                cur.close()
